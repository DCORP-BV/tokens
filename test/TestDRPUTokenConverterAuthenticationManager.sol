 ...
 * #author Frank Bonnet
 */  
contract TestDRPUTokenConverterAuthenticationManager {

  CallProxyFactory private callProxyFactory;

  function TestDRPUTokenConverterAuthenticationManager() {
  ...
  function test_Authentication_Is_Enabled_Initially() {
    ...
    // Arrange
    DRPUTokenConverter converter = new DRPUTokenConverter(address(0), address(0), address(0));
  ...
  function test_Non_Owner_Cannot_Disable_Authentication() {
    ...
    // Act
    DRPUTokenConverter(proxy).disableAuthentication();  // msg.sender will be the proxy
  ...
  function test_Non_Owner_Cannot_Enable_Authentication() {
    ...
    // Act
    DRPUTokenConverter(proxy).enableAuthentication();
    bool hasThrown = proxy.throws();
    bool isAuthenticatingAfter = converter.isAuthenticating();
